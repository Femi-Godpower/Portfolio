...
interface PageProps {
  readonly params: Promise<{ readonly segment: string; readonly locale: string }>;
  readonly searchParams: Promise<{ readonly email?: string; readonly token?: string }>;
}

export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const { segment, locale } = await params;
  const resolved = await resolveAuthFeaturePage({ feature: "resetPassword", variant: "country-language", countrySegment: segment, localeSegment: locale });
